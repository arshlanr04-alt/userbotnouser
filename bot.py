...
async def send_mirrored_content(client, tid, messages, default_t_topic, is_mir, sid, pre_downloaded=None):
    ...
    try:
        ...
        if not album_text and not file_to_send:
            logger.warning("MIRROR: Skipping empty message (no text and no media)")
            return
    ...
    try:
        ...
        while idx < len(items):
            ...
            try:
                ...
                if not getattr(msg, "message", "") and not getattr(msg, "media", None):
                    logger.warning(f"RELEASE: Skipping empty message {msg.id} (no text and no media)")
                    with db_conn() as conn:
                        c = conn.cursor()
                        p = get_placeholder()
                        if USING_POSTGRES:
                            c.execute("UPDATE collected_media SET released = 1 WHERE source_chat_id = %s AND source_message_id = %s", (int(sid_ref), int(msg.id)))
                        else:
                            c.execute("UPDATE collected_media SET released = 1 WHERE source_chat_id = ? AND source_message_id = ?", (int(sid_ref), int(msg.id)))
                        conn.commit()
                    continue
